...
ngx_uint_t    ngx_inherited;    //继承标识位
ngx_uint_t    ngx_daemonized;   //daemon标志位
 ...
 *  
 *  tips:
 *  master进程做的主要工作是启动woker进程，并根据信号来管理worker进程，而事件处理则交给woker进程来处理
 *  进程间通过socketpair(一对全双工的无名socket)进行通信。master进程与外部之间是通过信号通信的
 */
void
...
ngx_worker_process_init(ngx_cycle_t *cycle, ngx_int_t worker)
{
    ...
    /**
     * 获取进程的有效UID
     * 若是以root身份运行，则将worker进程降级为ccf->user 级 ccf->group权限. 默认是nobody。
     */
    if (geteuid() == 0) {
    ...
    //worker进程解除所有阻塞信号的功能，即不阻塞所有信号
    if (sigprocmask(SIG_SETMASK, &set, NULL) == -1) {
        ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno,
                      "sigprocmask() failed");
    }

    srandom((ngx_pid << 16) ^ ngx_time());  //设置种子值，进程ID+时间
    ...
    //调用所有模块的init_process方法来初始化各模块的核心功能
    for (i = 0; ngx_modules[i]; i++) {
        /**
         *  \file ../../event/ngx_event.c|
